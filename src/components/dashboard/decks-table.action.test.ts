...
describe("getDecksAction", () => {
  ...
  it("builds SQL with search and respects limit clamp; default order for 'all'", async () => {
    ...
    expect(sql).toMatch(/d\.name ilike \$2/i); // search bind
    // "all" sort
    expect(sql).toMatch(
...
describe("createDeckAction", () => {
  ...
  it("creates and maps return with cards=0", async () => {
    mockedAuth.mockResolvedValueOnce("u1");
    ...
    expect(params).toEqual(["Deck Name", "u1"]);
  });
});
...
describe("updateDeckAction", () => {
  ...
  it("updates and returns row; throws when not found", async () => {
    mockedAuth.mockResolvedValue("u1");
    ...
    const sql = mockedDb.mock.calls[
      mockedDb.mock.calls.length - 1
    ][0] as string;
    const params = mockedDb.mock.calls[
      mockedDb.mock.calls.length - 1
    ][1] as any[];
    expect(sql).toMatch(/update public\.decks/i);
    expect(params).toEqual(["New Name", "d1", "u1"]);
  });
});
...
describe("deleteDeckAction", () => {
  ...
  it("deletes and returns id; throws when not found", async () => {
    mockedAuth.mockResolvedValueOnce("u1");
    // missing id
...
describe("createFlashcardAction", () => {
  ...
  it("checks ownership, inserts with normalized difficulty, and touches deck.updated_at", async () => {
    mockedAuth.mockResolvedValue("u1");
    ...
    // ownership true, insert returns row
    ...
    mockedDb.mockResolvedValueOnce([] as any); // touch update

    const row = await createFlashcardAction({
      deckId: "d1",
      front: "Fr",
      back: "Bk",
      difficulty: "10", // should coerce to '5'
    });
    expect(row.id).toBe("c1");

    // check that the insert call used coerced difficulty "5"
    // check that the INSERT used coerced difficulty "5"
    const insertCall = mockedDb.mock.calls.find(
      ([sql]: [any]) =>
        typeof sql === "string" &&
        /insert into public\.flashcards/i.test(sql as string)
    );
    expect(insertCall).toBeTruthy();
    const insertParams = (insertCall as any)[1] as any[];
    expect(insertParams).toEqual(["d1", "Fr", "Bk", "5"]);

    // last call updates deck timestamp
    const lastSql = mockedDb.mock.calls[
      mockedDb.mock.calls.length - 1
    ][0] as string;
...
describe("updateFlashcardAction", () => {
  ...
  it("updates with difficulty coercion and touches deck", async () => {
    ...
    // verify params contained coerced difficulty "5"
    const updateParams = mockedDb.mock.calls[
      mockedDb.mock.calls.length - 2
    ][1] as any[];
...
describe("deleteFlashcardAction", () => {
  ...
  it("deletes with user ownership join and touches deck; throws when not found", async () => {
    mockedAuth.mockResolvedValue("u1");
    ...
    // success
    mockedDb.mockResolvedValueOnce([{ id: "c1" }] as any);
    mockedDb.mockResolvedValueOnce([] as any); // touch deck
    const res = await deleteFlashcardAction({ id: "c1", deckId: "d1" });
    expect(res).toEqual({ id: "c1" });

    // check SQL shape
    const deleteSql = mockedDb.mock.calls[
      mockedDb.mock.calls.length - 2
    ][0] as string;
    const deleteParams = mockedDb.mock.calls[
      mockedDb.mock.calls.length - 2
    ][1] as any[];
